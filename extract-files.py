...
# =========================== AUTO DUPLICATE / SYMLINK HANDLER =============================
...
def detect_duplicate_blobs(blob_list):
    seen = {}         # md5hash → first blob
    final_blobs = []  # fixed list

    for blob in blob_list:
        real_blob = blob.split(";")[0]
        real_path = os.path.join("proprietary", real_blob)

        blob_hash = hash_file(real_path)

        if blob_hash:
            if blob_hash in seen:
                original = seen[blob_hash]
                print(f"[SYMLINK] {real_blob} → {original}")
                final_blobs.append(f"{real_blob};SYMLINK={original}")
            else:
                seen[blob_hash] = real_blob
...
blob_fixups: blob_fixups_user_type = {
    ('odm/lib64/libPanelChaplin_mtk.so', 'vendor/lib64/mt6895/libmtkcam_stdutils.so'): blob_fixup()
...
if __name__ == '__main__':
    print("\n[CHECK] Detecting duplicate blobs via MD5...")
    with open('proprietary-files.txt', 'r') as f:
        blobs = [line.strip() for line in f if line.strip() and not line.startswith('#')]

    blobs_fixed = detect_duplicate_blobs(blobs)

    with open('proprietary-files.txt', 'w') as f:
        for line in blobs_fixed:
            f.write(line + "\n")
